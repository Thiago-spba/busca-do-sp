...
import { Star, FolderKanban, ChevronDown, ChevronUp, RefreshCw, Trash2, X, Plus } from "lucide-react";

function SecaoLista({ nome, ehPadrao, itens, onAtualizarLote, onRemoverDaLista, onExcluirLista, progressoLote, bloqueado }) {
  ...
  const handleAtualizarTodos = async () => {
    setResumo(null);
    const resultados = await onAtualizarLote(itens);
    const mapa = {};
  ...
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: "10px", overflow: "hidden" }}>
      ...
      >
        {ehPadrao ? <Star size={16} /> : <FolderKanban size={16} />}
        {nome} ({itens.length})
      ...
      {aberto && (
        <div style={{ padding: "0 1rem 1rem", display: "flex", flexDirection: "column", gap: "0.6rem" }}>
          {itens.length === 0 ? (
            <p style={{ color: "var(--text-muted)", fontSize: "0.8rem" }}>Nenhuma pessoa nesta lista ainda.</p>
          ) : (
            <>
                ...
                return (
                  <div key={entrada.id} style={{ background: "var(--bg-app)", border: "1px solid var(--border-color)", borderRadius: "8px", padding: "0.6rem 0.85rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                      ...
                      <button
                        onClick={() => onRemoverDaLista(entrada.id)}
                        disabled={bloqueado}
...
export function MinhasListas({ nomesListas, itensListas, onAtualizarLote, onRemoverDaLista, onCriarLista, onExcluirLista, progressoLote, bloqueado }) {
  ...
  const secoes = ["Favoritos", ...nomesListas];
    ...
    <div style={{ marginTop: "1.5rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
      ...
      {secoes.map((nome) => (
        <SecaoLista
          key={nome}
          nome={nome}
          ehPadrao={nome === "Favoritos"}
          itens={itensListas.filter((b) => b.lista === nome)}
          onAtualizarLote={onAtualizarLote}
          onRemoverDaLista={onRemoverDaLista}
